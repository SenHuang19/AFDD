...
model CooLoa
  ...
  Buildings.Utilities.IO.BCVTB.From_degC toDegC1
    annotation (Placement(transformation(extent={{36,-26},{56,-6}})));
  Buildings.Utilities.IO.BCVTB.From_degC toDegC2
    annotation (Placement(transformation(extent={{34,60},{54,80}})));
  Modelica.Blocks.Interfaces.RealOutput TOutDryBul "Temperature in Kelvin"
...
equation
  ...
  connect(add.u2, building_fmu_fmu1.T_in) annotation (Line(points={{26,-52},{14,
          -52},{14,0},{8,0}},  color={0,0,127}));
  connect(building_fmu_fmu1.m, product.u1) annotation (Line(points={{8,0},{38,0},
          {38,36},{50,36}},     color={0,0,127}));
  ...
  connect(gain.y, Loa) annotation (Line(points={{88.4,30},{96,30},{96,0},{110,0}},
        color={0,0,127}));
  connect(building_fmu_fmu1.CHW_temp, toDegC.Celsius)
    annotation (Line(points={{-12.4,0},{-35,0}}, color={0,0,127}));
  connect(toDegC.Kelvin, CHW_temp)
    annotation (Line(points={{-58,0},{-120,0}}, color={0,0,127}));
  connect(toDegC1.Celsius, building_fmu_fmu1.TOutWetBul) annotation (Line(
        points={{34,-16.4},{26,-16.4},{26,0},{8,0}},       color={0,0,127}));
  connect(toDegC1.Kelvin, TOutWetBul) annotation (Line(points={{57,-16.2},{80,
          -16.2},{80,-40},{110,-40}}, color={0,0,127}));
  connect(building_fmu_fmu1.TOutDryBul, toDegC2.Celsius) annotation (Line(
        points={{8,0},{26,0},{26,69.6},{32,69.6}},       color={0,0,127}));
  connect(toDegC2.Kelvin, TOutDryBul) annotation (Line(points={{55,69.8},{66,
          69.8},{66,60},{110,60}}, color={0,0,127}));
